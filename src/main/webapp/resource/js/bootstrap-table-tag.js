function InitMainTagTable() {
    $("#back_tag_table").bootstrapTable({
        ...
        columns: [
            ...
            //暂时不添加，sql查询语句未完善
            //{fiele: 'article_count', title: '文章数量'},
...
//新增标签
function addTag(){
    var param = $("#addTagModal").serializeArray();
    debugger;
...
function beforOpenEditModal() {
    ...
    setTimeout(function () {
        //alert(ths.parentsUntil("input").val);
        //alert($(row).html());
        openEditModal();
    },1000);
}
...
/*function openEditModal_simple(row) {
    $('#edit_input1').val(row.tag_name);
    $('#edit_input2').val(row.tag_content);
    $('#edit_input3').val(row.tag_cid);
    $("#editTagModal").modal("show");
...
function editTag(){
    var param = $("#editTagForm").serializeArray();
    //设为disable则无法获取
    $.ajax({
        url:"/updateTag",
        method:"post",
